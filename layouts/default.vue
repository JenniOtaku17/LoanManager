    ...
    >
      <v-list>
        <v-list-item class="px-2 logoContainer">
          <img src="~assets/images/logo.png" :width="miniVariant?'100%':'60%'" /> 
        </v-list-item>
        ...
        <v-list-item v-on:click="logOut">
          <v-list-item-action>
            <v-icon :color="$vuetify.theme.dark? 'white':'secondary'">mdi-logout</v-icon>
          </v-list-item-action>
          <v-list-item-content>
      ...
      <template v-slot:append >
        ...
        <div class="pa-2" v-else>
          <span class="version">V {{ version }}</span><br>
        </div>
      </template>
    </v-navigation-drawer>
    <v-app-bar
